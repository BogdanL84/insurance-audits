...
11. ~~**Synthesis prompt should include a PROGRAM INVENTORY block in every chunk.**~~ **RESOLVED 2026-05-10** by *"Synthesis prompt: PROGRAM INVENTORY block prevents chunk-blind hallucinations (item 11)"* — added a medium-detail PROGRAM INVENTORY block (filename + policy_type + coverage_parts) to `build_crossref_prompt` in `claude_runner.py` (~L1432, between `CLIENT SLUG:` and `CONTRACT REQUIREMENTS:`). New parameter `full_program_inventory: list | None = None` on the function; chunking.py call sites in `run_chunked_synthesis` plumb the full `policy_analyses` list (not the chunk subset) so the model sees the whole program even when analyzing only one cluster. Rule text has two parts: a "Do NOT emit X-missing findings when X is in inventory" guardrail, and a "MUST still emit Program Gap findings for genuinely-absent coverages" carve-out listing standard commercial coverages (Cyber, Crime, Pollution, Pro Liability/E&O, Products, D&O, Stop Gap, Standalone Umbrella). First draft was rebalanced after RUN1 over-suppressed legitimate gaps; RUN2 with explicit MUST-emit list verified clean. Verified on full Precision Aero pipeline: filter drop count 0 (vs 1 original), hallucination shapes 0, items 12+13 regressions clean, all 6 legitimate missing-coverage findings present, total 122 findings (within variance band). Original 2026-05-08 deferral context retained below for record:

    *Original issue (deferred):* Each chunked synthesis call currently sees only the policies in its coverage cluster, which lets the model emit "Program Gap — No Workers' Comp Policy" from the core-liability chunk because *that chunk* doesn't see WC PEKIN. Defensive filter at `core/findings_filter.py` catches the canonical hallucinations post-merge; upstream fix is the PROGRAM INVENTORY block.
...
14. **Hallucination filter pattern gap — "Program-Level Coverage Gap — X Not in Audit Scope" shape.** *(LOW PRIORITY — defense-in-depth)* The existing filter at `core/findings_filter.py` (added 2026-05-08) catches the canonical chunk-induced hallucination shapes via `^Program Gap.*\bNo X\b` and `^Missing Policy.*X` regex. During item 13 verification on 2026-05-09 (single-WC synthesis test), the model emitted a NEW phrasing the filter does not catch: `Program-Level Coverage Gap — General Liability Policy Not in Audit Scope` (and similar). Same single-policy-chunk artifact, new title shape. **Status as of 2026-05-10: the upstream fix is now in place.** Item 11's PROGRAM INVENTORY block prevents this entire class of hallucination at the source — verified on full Precision Aero pipeline (RUN2 emitted zero hallucination-shape findings across all 4 chunks, filter drop count 0). Filter pattern extension is no longer the primary defense; it remains open as low-priority defense-in-depth in case a future client / prompt change surfaces a new shape that leaks past the inventory rule. Defer indefinitely unless a real leak appears.
