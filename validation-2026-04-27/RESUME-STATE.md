...
12. **Synthesis prompt emits "Audit Scope Limitation" meta-findings when no contract requirements are loaded.** When the audit runs in standalone mode (no contracts uploaded), per-policy synthesis emits a Needs-Review meta-finding flagging the audit-scope limitation ("The contract requirements file passed into this audit was empty"). This belongs in audit-state metadata or the report cover-page, not the findings list — it's a fact about the audit process, not about the policy. Suppress at the prompt level: instruct the synthesis prompt to NOT emit findings about the audit process itself; surface "no contracts supplied" context elsewhere (state metadata, cover-page subtitle). Manually dropped on 2026-05-08 for Precision Aero (drop record in `audit-state.json:filter_drops`). Defer until next prompt-tightening pass.

13. **Synthesis prompt classifies "Pending Rate Change — Mid-Term Premium Risk" as Needs Review when it should be Bad.** The Pending Rate Change endorsement (e.g. WC 00 04 04 in NCCI WC forms) permits the carrier to adjust rates mid-term once a state filing is approved. This is a concrete policy provision with material premium-impact downside (premium not fully fixed for the term), not a confirm-with-carrier checklist item. The model classified it as Needs Review on Precision Aero. Tighten the prompt rule: "carrier may adjust rate mid-term once a state-filing is approved" → Bad-category finding (premium volatility risk). Manually reclassified on 2026-05-08 for Precision Aero (severity 3 × likelihood 3 = risk 9). Defer until next prompt-tightening pass.
